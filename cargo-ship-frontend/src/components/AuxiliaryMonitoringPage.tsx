/**
 * AuxiliaryMonitoringPage 组件 - 辅助系统监控页面（重构版）
 *
 * 功能说明：
 * - 采用监控墙模式重构的辅助系统监控页面
 * - 整合舱底水系统和冷却水系统的实时监控数据
 * - 使用MonitoringWall组件实现响应式网格布局
 * - 使用DedicatedAlarmZone组件显示辅助系统专属告警
 * - 管理实时数据订阅的生命周期（订阅/取消订阅）
 *
 * 页面布局：
 * - 顶部：页面标题和面包屑导航
 * - 中部：左右双栏监控墙（响应式网格展示所有监测点）
 *   - 左栏：舱底水系统（SYS-BILGE-001）
 *   - 右栏：冷却水泵系统（SYS-COOL-001）
 * - 底部：专属告警区（仅显示辅助系统的告警）
 *
 * 架构特点：
...
import { Settings, Home, ChevronRight, Wifi, WifiOff, Loader2 } from 'lucide-react';
 ...
 * 舱底水系统设备ID
 */
const BILGE_EQUIPMENT_ID = 'SYS-BILGE-001';

/**
 * 冷却水泵系统设备ID
 */
const COOLING_EQUIPMENT_ID = 'SYS-COOL-001';

/**
 * 页面标题
 */
const PAGE_TITLE = '辅助系统监控';
...
const BREADCRUMBS = [
  ...
  { label: PAGE_TITLE, path: '/monitoring/auxiliary' },
];
...
// ============================================================================
// AuxiliaryMonitoringPage 组件
// ============================================================================

/**
 * AuxiliaryMonitoringPage - 辅助系统监控页面
 *
 * 采用监控墙模式的标准实现，使用左右双栏布局展示舱底水和冷却水系统监控
 */
export const AuxiliaryMonitoringPage: React.FC = () => {
  // --------------------------------------------------------------------------
  // 状态管理
  // --------------------------------------------------------------------------
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900/20 to-gray-900 p-6">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-300 mt-1 font-medium italic">舱底水和冷却水系统实时监控与管理</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          >
            <MonitoringWall
              equipmentId={BILGE_EQUIPMENT_ID}
              title="舱底水系统"
          ...
          >
            <MonitoringWall
              equipmentId={COOLING_EQUIPMENT_ID}
              title="冷却水泵系统"
              columnCount={{ desktop: 2, tablet: 2, mobile: 1 }}
              className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50"
            />
        ...
        >
          <DedicatedAlarmZone
            equipmentIds={[BILGE_EQUIPMENT_ID, COOLING_EQUIPMENT_ID]}
            maxItems={10}
            className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50"
          />
        </motion.div>
...
export default AuxiliaryMonitoringPage;
